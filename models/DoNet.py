...
from .nets.SCINet import SCINet
...
class DoNet(nn.Module):
    ...
    def infer(self, input):
        masks = self.net(input)
        return masks[0]
    

    def backward_G(self):    
        self.loss_G = 0.5 * self.criterionBCE(self.pred[0], self.gt_mask)
        self.loss_G += self.criterionDice(self.pred[0], self.gt_mask)
